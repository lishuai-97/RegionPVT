...
from data_util_test import sa_create, collate_fn
from data_util_test import data_prepare_v101 as data_prepare
...
if __name__ == '__main__':
    # data_root = '/home/lishuai375/data/stanford_indoor3d'
    data_root = '/home/sun/Desktop/ShuaiLi/stanford_indoor3d_test'
    # test_area, voxel_size, voxel_max = 5, 0.04, 80000
    test_area, voxel_size, voxel_max, batch_size = 5, 0.04, 4096, 1
    ...
    train_loader = torch.utils.data.DataLoader(point_data, batch_size=batch_size, shuffle=False, num_workers=0, pin_memory=True, collate_fn=collate_fn)
    for idx in range(1):
        end = time.time()
        voxel_num = []
...
#             print('tag', coord.shape, feat.shape, label.shape, offset.shape, torch.unique(label))
            ...
            print(f'tag coord shape: {coord.shape}, feat shape: {feat.shape}, label shape: {label.shape}, offset shape: {offset.shape}, torch.unique(label): {torch.unique(label)}')
