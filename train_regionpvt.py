...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    # get model
    if args.arch == 'regional_to_local_point_voxel_transformer':
        
        from model.regionpvt import RegionPVT
        print('Will use model: Regional-to-Local Point-Voxel Transformer !')
        ...
        model = RegionPVT(args.depths, args.channels, args.num_heads, \
...
#         model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[gpu], find_unused_parameters=False)
    ...
    if args.data_name == 's3dis':
        ...
        # val_data = S3DIS(split='val', data_root=args.data_root, test_area=args.test_area, voxel_size=args.voxel_size, voxel_max=args.voxel_max, transform=val_transform)      # voxel_max=5000 for calcualte FLOPs, Params and Memeory
    elif args.data_name == 'scannetv2':
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch_log % args.save_freq == 0) and main_process():
            ...
            logger.info('Currently Best mIoU: {:.4f}'.format(best_iou))
